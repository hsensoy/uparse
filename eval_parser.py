...
class ParseEvaluator:
  ...
  def compute_fscore(self, key_trees, predicted_trees):
    for trees in zip(key_trees, predicted_trees):
      ...
      if len(f1) != len(f2): 
        raise ParseError("Sentence length does not match. Gold sentence length %d, test sentence length %d. Sentence '%s' vs '%s'"%(len(f1), len(f2), " ".join(f1), " ".join(f2)))
...
def main(key_file, prediction_file):
    key_trees,predicted_trees = [],[]
    nmissed = 0
    for key_line, predicted_line in zip(key_file, prediction_file):
        if predicted_line.strip() != '#####':
            key_trees.append(json.loads(key_line))
            predicted_trees.append(json.loads(predicted_line))
        else:
            nmissed += 1
    ...
    print "%d trees could not be formed"%nmissed
