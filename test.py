...
class TestUparse(unittest.TestCase):
	...
	def test_edge_count_in_coNLL10(self):
		for dg in alldgiter('coNLL10.dp'):
			self.assertEqual(dg.length(), len(dg.edgeset(False)))
	...
	def test_lhead_metrics(self):
		...
		for dg in  dgiter('conll/00/wsj_0001.dp'):
			m_ignoreroot.add(dg, lhead(dg))
			m.add(dg, lhead(dg))

		self.assertEqual(31, m.gNedge)
		self.assertEqual(29, m_ignoreroot.gNedge)
		self.assertEqual(15, m_ignoreroot.uNmatch)
		self.assertEqual(7, m_ignoreroot.dNmatch)
		self.assertEqual(15, m.uNmatch)
		self.assertEqual(7, m.dNmatch)
	...
	def test_rhead_metrics(self):
		...
		self.assertEqual(29, m_ignoreroot.gNedge)
		self.assertEqual(15, m_ignoreroot.uNmatch)
		self.assertEqual(8, m_ignoreroot.dNmatch)
		self.assertEqual(15, m.uNmatch)
		self.assertEqual(8, m.dNmatch)
		
	
	def test_dependency_graph(self):
		from sets import Set
		for i, dg in enumerate(dgiter('conll/00/wsj_0001.dp')):
			if i == 0:
	...
	def test_wsj10_sentence_count(self):
		self.assertEqual(7422, len([dg for dg in dgiter(wsj10file)]))
	
	def test_wsj10_sentence_length_freq(self):
		expected = {1: 159, 2: 340, 3: 377, 4: 518, 5: 614, 6: 737, 7: 878, 8: 1107, 9: 1208, 10: 1484}
		actual = {}
		for dg in dgiter(wsj10file):
